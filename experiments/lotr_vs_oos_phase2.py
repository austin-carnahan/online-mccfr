"""Experiment: Depth-LOTR vs OOS — Phase 2 headline comparison.

Apples-to-apples on prefix-divergence rate.  Two paired comparisons:

  Low divergence (ρ = 1−δ = 0.1):
    - oos_d09         OOS, δ = 0.9   (90% of episodes targeted)
    - lotr_r01        Depth-LOTR, ρ = 0.1, uniform depth profile

  High divergence (ρ = 1−δ = 0.5):
    - oos_d05         OOS, δ = 0.5   (50% of episodes targeted)
    - lotr_r05        Depth-LOTR, ρ = 0.5, uniform depth profile

Within each pair both arms have the same expected fraction of episodes
that stay on the full target prefix (δ for OOS, 1−ρ for LOTR).  All
other knobs identical (ε=0.4, γ=0.01).  LOTR uses a flat (uniform) depth
profile, so divergence is spread evenly across prefix depths rather than
allocated by a single global coin (OOS).

Games: leduc_poker, goofspiel, liars_dice (depth varies).
Budgets: 5k, 20k, 100k, 500k.
Matches: 100 (≤100k), 30 (500k).
Total: 3 games × 4 arms × 4 budgets = 48 jobs.

Usage:
    python -m experiments.lotr_vs_oos_phase2 [--workers 5] [--pilot]
"""
...
from src.depth_lotr import DepthLOTRBot, uniform
...
BUDGETS = [5_000, 20_000, 100_000, 500_000]
...
ALGORITHMS = ["oos_d05", "oos_d09", "lotr_r05", "lotr_r01"]

# Per-algorithm parameters.  Pairs share prefix-divergence rate:
#   oos_d09 ↔ lotr_r01  (10% divergence)
#   oos_d05 ↔ lotr_r05  (50% divergence)
OOS_DELTAS = {"oos_d05": 0.5, "oos_d09": 0.9}
LOTR_RHOS  = {"lotr_r05": 0.5, "lotr_r01": 0.1}

EPSILON = 0.4
GAMMA = 0.01
SEED = 42

OUTPUT_DIR = "results/lotr_vs_oos_phase2"
...
def _run_job(job: dict) -> dict:
    ...
    for match_idx in range(num_matches):
        ...
        if algo.startswith("oos"):
            ...
            schedule = uniform(LOTR_RHOS[algo])
...
def main():
    parser = argparse.ArgumentParser(description="Depth-LOTR vs OOS Phase 2")
    ...
    if args.pilot:
        games = ["leduc_poker"]
        global BUDGETS, MATCHES_BY_BUDGET
        BUDGETS = [5_000, 20_000]
        MATCHES_BY_BUDGET = {5_000: 10, 20_000: 10}
    jobs = build_jobs(games)
    output_dir = args.output
    os.makedirs(output_dir, exist_ok=True)

    total = len(jobs)
    print(f"Depth-LOTR vs OOS Phase 2 — {total} jobs")
    print(f"  Games: {games or GAMES}")
    print(f"  Algorithms: {ALGORITHMS}")
    print(f"    OOS δ: {OOS_DELTAS}")
    print(f"    LOTR ρ (uniform depth profile): {LOTR_RHOS}")
