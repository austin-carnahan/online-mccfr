"""Paired-seed Kuhn benchmark: LOTR step(0.5, 0) vs OOS delta=0.5.

This benchmark uses the canonical LOTR implementation rather than
``DepthLOTRBot``.
...
Usage:
    PYTHONPATH=. python -m experiments.lotr_step_vs_oos_kuhn --workers 8

Outputs:
    results/lotr_step_vs_oos_kuhn/incremental_results.jsonl
    results/lotr_step_vs_oos_kuhn/results.json
    results/lotr_step_vs_oos_kuhn/summary.csv
    results/lotr_step_vs_oos_kuhn/summary.md
"""
...
OOS_DELTA = 0.5
LOTR_RHO = 0.5
OUTPUT_DIR = "results/lotr_step_vs_oos_kuhn"
...
def _write_markdown(path, algorithm_rows, paired_rows, config):
    with open(path, "w") as handle:
        handle.write("# LOTR step vs OOS on Kuhn\n\n")
        handle.write("## Config\n\n")
        ...
        handle.write(
            "| sims_per_move | n | mean diff | median diff | p10 | p90 | mean 95% CI | LOTR better | oos better | ties |\n")
        handle.write("|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Paired-seed Kuhn sweep: LOTR step(0.5,0) vs OOS delta=0.5")
    ...
    print("LOTR step vs OOS on Kuhn")
