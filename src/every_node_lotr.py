"""Every-Node LOTR — τ-blend Local Online Target-Rate at every prefix node.

Generalization of Depth-LOTR.  Where Depth-LOTR applied its (ρ, w)
schedule only at *player decision* nodes on the prefix and treated
chance specially (forcing observable chance, sampling natural at
hidden chance), Every-Node LOTR applies the same τ(d) blend uniformly
at *every* non-terminal prefix node — chance nodes included — using
the natural action distribution as the "free" σ.  At a chance node
with outcome distribution π and target outcome a*:

    q(a*) = τ(d)
    q(a)  = (1 − τ(d)) · π(a) / (1 − π(a*))   for a ≠ a*

At a player node it is the same blend with σ_ε in place of π — the
formula is identical to Depth-LOTR's decision-node blend.  Sticky
divergence: missing the target at any prefix position (chance or
decision) sets diverged=True for the rest of the trajectory.

Why this is different from Depth-LOTR:

  * Depth-LOTR forces every observable chance outcome, so an episode
    that draws the wrong card on a public chance node never happens —
    counterfactual coverage of off-prefix chance branches comes only
    from divergences at later player nodes.  On games where the
    bottleneck is chance-branch coverage (e.g. Kuhn at high budgets,
    where the leaked-information plateau is dominated by P0's hidden
    deal), this caps the asymptote.

  * OOS-IST has a per-iteration coin: with prob 1−δ the whole iter is
    untargeted and resamples all chance freely.  This gives uniform
    counterfactual coverage but couples decision-prefix and chance-
    prefix divergence under one knob.

  * Every-Node LOTR has a *per-node* coin at every prefix position,
    decision and chance alike, so the depth profile w(d) controls
    where the divergence budget gets allocated across the whole
    history — not just across player decisions.  At ρ=1, w=step(0):
    the root chance deal itself is τ-blended at rate τ(0)=1−ρ.
...
src.depth_lotr.  Their tau(d, D) signature is unchanged but the meaning
of D is now the *total prefix length* (chance + decisions), not just
decision count.
"""
...
class EveryNodeLOTRBot(pyspiel.Bot):
    """Every-node LOTR bot — τ-blend at every prefix node (chance + decision)."""
    ...
    def __init__(self, game, player_id, num_simulations=1000,
        ...
        self._match_history = []
        # Position index along the prefix (0..D-1), defined for every
        # non-terminal prefix node — chance and decision alike.
    ...
    def _reset_tracking(self):
        ...
        # realized prefix node (chance + decision) matched the
        ...
        # Keyed by prefix position d (0..D-1), spanning chance + decision.
    ...
    def get_tracking_summary(self):
        ...
        return {
            ...
            # Unified apples-to-apples metric: fraction of episodes
            # whose realized actions matched the match-history target
            # at every prefix position (chance + decision).
            "prefix_survival_fraction": float(prefix_arr.mean()),
            "mean_diverge_depth": float(div_arr[div_arr >= 0].mean()) if (div_arr >= 0).any() else -1.0,
            # D = total prefix length (chance + decisions).
    ...
    def _walk(self, state, update_player, my_reach, opp_reach, l, diverged):
        ...
        if not in_tree:
            ...
            # Unified prefix-survival tracking.
    ...
    def _handle_chance(self, state, update_player, my_reach, opp_reach,
        ...
        # Determine whether this chance node lies on the prefix and is a
        # candidate for τ-blending.
        ...
        if in_prefix:
            ...
            # Off-prefix or already diverged: sample chance from natural π.
            sampled_idx = self._rng.choice(num_actions, p=probs_arr)
            action = actions[sampled_idx]
            rho = probs_arr[sampled_idx]
            new_diverged = diverged
            in_history = depth < len(self._match_history)
            if (not diverged) and in_history and action != self._match_history[depth]:
                # Should not normally hit this (in_prefix would be true);
                # safety: treat as divergence.
                new_diverged = True
                if self._tracking:
                    self._cur_chance_diverged = True
                    self._cur_prefix_survived = False
    ...
    def _build_targeting_info(self, current_state):
        """Walk the match history once, populating _depth_to_position at
        every non-terminal prefix position (chance + decision).

        D = total prefix length.  No IST observability test is performed —
        every prefix node is τ-blended uniformly, so chance nodes are
        treated identically to decision nodes.
        """
    ...
    def average_policy(self):
        return EveryNodeLOTRPolicy(self._game, self._infostates)


class EveryNodeLOTRPolicy:
    """Adapter so EveryNodeLOTRBot info states look like an OpenSpiel Policy."""
...
# Re-export schedule constructors for convenience.
__all__ = [
    "EveryNodeLOTRBot",
    "EveryNodeLOTRPolicy",
    "LOTRSchedule",
