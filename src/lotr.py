"""LOTR — per-node OOS-style coin at every observable prefix node.

Refines Observable-Node LOTR (v3) by replacing the redistribution
τ-blend at the divergence coin with an OOS-style targeting coin and
...
This is the per-iter arm marginal under the LOTR sampling distribution,
generalizing OOS's δ·s1 + (1−δ)·s2 to D+1 scenarios.
...
positions (contributing to D and gating the targeting coin).  Player
decisions are always observable.  Built-in oracles:
    ...
    - EveryChanceObservable — every chance node counts (recovers v2's
        chance treatment under the targeting coin).
...
──────────────────────────────────────────────────────────────────────
(ρ, w) parameterization
──────────────────────────────────────────────────────────────────────

Schedules are parameterized by a total divergence budget ρ ∈ [0,1] and
a shape w over backoff position k = D − d (distance from active state):

    S(d) = Π_{j<d} τ(j) = 1 − ρ · Σ_{j<d} w(j)
    τ(d) = 1 − ρ · w(d) / S(d)

  - P(diverge somewhere on prefix) = ρ   (per-state guarantee)
  - P(backoff=k | diverged, D)     = w(k) / Σ_j w(j)

Built-in shapes (in depth coordinates d, with k = D − d the backoff
distance from the active state):
  - uniform(ρ)         w(d) = 1            (flat over the prefix)
  - early_linear(ρ)    w(d) = D − d        (mass at the root, decays toward active)
  - late_linear(ρ)     w(d) = d + 1        (mass at active, decays toward root)
  - step(ρ, depth=k)   w(d) = δ_{d,k}      (single coin at depth k;
                                            step(ρ, depth=0) ≡ OOS-targeted
                                            coin at the root)
...
# ═════════════════════════════════════════════════════════════════════════
# Schedules  —  (ρ, w) parameterization
# ═════════════════════════════════════════════════════════════════════════
...
class WeightedExplorationSchedule(LOTRSchedule):
    ...
    τ(d) is derived from total divergence budget ρ and a shape w over the
    prefix.  weight_fn(d, D) returns nonnegative raw weights; they are
    normalized per D into w(d), and τ is computed once per D and cached.

    Built-in shapes index w in backoff coordinates k = D − d (distance
    from the active state), so the conditional landing distribution
    P(backoff=k | diverged, D) = w(k) / Σ_j w(j) has a fixed, D-invariant
    interpretation.
    """
...
def uniform(rho):
    """Uniform depth shape: w(d) = 1.

    P(backoff=k | diverged, D) = 1/D for k = 1..D.
    """
...
def early_linear(rho):
    """Early-linear depth shape: w(d) = D − d.

    Mass concentrates near the root (early depths); in backoff coords
    that is far backoff (large k).
    """
    return WeightedExplorationSchedule(
        rho, lambda d, D: float(D - d), f"early_linear(ρ={rho})"
    )


def late_linear(rho):
    """Late-linear depth shape: w(d) = d + 1.

    Mass concentrates near the active state (late depths); in backoff
    coords that is near active (small k).
    """
    return WeightedExplorationSchedule(
        rho, lambda d, D: float(d + 1), f"late_linear(ρ={rho})"
    )


def step(rho, depth=0):
    """Step depth profile: all divergence concentrated at one depth d.

    step(ρ, depth=0) recovers the OOS-targeted coin at the root
    (equivalent to backoff=D).
    """
    return WeightedExplorationSchedule(
        rho,
...
class LOTRBot(pyspiel.Bot):
    """LOTR bot — per-node OOS-style coin at every observable position."""
    ...
    def step_with_policy(self, state):
        ...
        for sim in range(self._num_simulations):
            ...
            if self._tracking:
                ...
                self._cur_coin_diverge_depth = -1
                self._cur_coin_diverge_kind = None
    ...
    def _reset_tracking(self):
        ...
        self._iter_coin_diverge_depth = []
        self._iter_coin_diverge_kind = []
    ...
    def _record_coin_divergence(self, d):
        if self._cur_coin_diverge_depth != -1:
            return
        self._cur_coin_diverge_depth = d
        self._cur_coin_diverge_kind = self._prefix_position_kind.get(d, "unknown")
    ...
    def get_tracking_summary(self):
        ...
        coin_div_arr = np.array(self._iter_coin_diverge_depth)
        ...
        coin_diverge_depth_counts = {}
        coin_diverge_depth_kind_counts = {}
        coin_diverged_mask = coin_div_arr >= 0
        coin_diverged_count = int(coin_diverged_mask.sum())
        for depth, kind in zip(self._iter_coin_diverge_depth,
                               self._iter_coin_diverge_kind):
            if depth < 0:
                continue
            depth_key = str(int(depth))
            kind_key = kind if kind is not None else "unknown"
            coin_diverge_depth_counts[depth_key] = (
                coin_diverge_depth_counts.get(depth_key, 0) + 1
            )
            if depth_key not in coin_diverge_depth_kind_counts:
                coin_diverge_depth_kind_counts[depth_key] = {}
            coin_diverge_depth_kind_counts[depth_key][kind_key] = (
                coin_diverge_depth_kind_counts[depth_key].get(kind_key, 0) + 1
            )

        prefix_position_kind_counts = {}
        for kind in self._prefix_position_kind.values():
            prefix_position_kind_counts[kind] = (
                prefix_position_kind_counts.get(kind, 0) + 1
            )
        ...
        return {
            ...
            "coin_diverge_fraction": coin_diverged_count / n_iters,
            "coin_no_divergence_count": n_iters - coin_diverged_count,
            "coin_diverge_depth_counts": coin_diverge_depth_counts,
            "coin_diverge_depth_kind_counts": coin_diverge_depth_kind_counts,
            "mean_coin_diverge_depth": float(coin_div_arr[coin_diverged_mask].mean()) if coin_diverged_count else -1.0,
            "prefix_D": int(self._D),
            "prefix_position_kinds": {
                str(int(depth)): kind
                for depth, kind in sorted(self._prefix_position_kind.items())
            },
            "prefix_position_kind_counts": prefix_position_kind_counts,
    ...
    def _episode(self, state, update_player):
        ...
        if self._tracking:
            ...
            self._iter_coin_diverge_depth.append(self._cur_coin_diverge_depth)
            self._iter_coin_diverge_kind.append(self._cur_coin_diverge_kind)
    ...
    def _walk(self, state, update_player, my_reach, opp_reach, s, diverged):
        ...
        if in_prefix:
            ...
            if self._tracking and miss:
                self._cur_prefix_survived = False
                self._record_coin_divergence(d)
                if self._cur_diverge_depth == -1:
                    self._cur_diverge_depth = d
        else:
        ...
        for d in range(n):
            ...
            if s.is_chance_node():
                if self._observability.is_observable_chance(
                    ...
                    self._prefix_position_kind[pos] = "chance"
                    pos += 1
                # Hidden chance: skip (no position assigned).
            else:
                cur_player = s.current_player()
                self._depth_to_position[d] = pos
                self._prefix_position_kind[pos] = f"p{cur_player}"
                pos += 1
    ...
    def average_policy(self):
        return LOTRPolicy(self._game, self._infostates)


class LOTRPolicy:
    """Adapter so LOTRBot info states look like an OpenSpiel Policy."""
